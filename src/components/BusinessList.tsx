...
export const BusinessList: React.FC<BusinessListProps> = ({ businessList }) => {
  ...
  // (x, y)
  filteredBusinesses.sort((business1: BusinessEntry, business2 : BusinessEntry) => { 
    ...
    if (business1.Initiatives?.length < business2.Initiatives?.length) return 1;
    if (business2.Initiatives?.length < business1.Initiatives?.length) return -1;
    else return 1; 
  })
